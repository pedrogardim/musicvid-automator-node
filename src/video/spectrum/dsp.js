import { createSpectrumAnalyzer } from './spectrum.js';

export const brutForceFFTSignalLength = (PCMDataLength) => {
  let exponent = 1;
  while (true) {
...
export const getSpectrum = (PCMData) => {
  ...
  const signal = PCMData.slice(
    signalStartIndex,
    signalStartIndex + signalLength
  );
  ...
  for (let i = 0; i < spectrum.length; i++) {
    ...
    spectrum[i] *=
      -1 *
      Math.log((signalLength / 2 - i) * (0.5 / signalLength / 2)) *
      signalLength;
  }
  return spectrum;
};
